...
if !(Equal immediateTempVar op_baitdir_overshoot) && !(CalledAs FastAerial) && Equal currGoal cg_attack
...
if Equal priority priority_bait && !(Equal lastAttack -1)
  Return
...
label combo_check
  ...
  MOVE_KB_WITHIN(result, move_currKnockback, move_angle, 65, 0, 125, -80, 185)
  if Equal result 1
    DynamicDiceAdd lastAttack
...
label juggle_check
  ...
  if move_angle >= 50 && move_angle <= 130
    MOVE_KB_WITHIN(result, move_currKnockback, move_angle, 30, 0, 60, 0, 185)
    if Equal result 1
      DynamicDiceAdd lastAttack
      Goto fastCheck
      Goto dirCheck
...
label pressure_check
  if move_currKnockback >= 25 && move_currKnockback <= 85
    DynamicDiceAdd lastAttack
...
Return
label space_check
  if hitFrame > 10 || move_currKnockback < 60
  elif move_IASA > 30
  elif duration < 6
  $elifLastAttack(uthrow)
  $elifLastAttack(fthrow)
...
label launch_check
  ...
  if move_angle > 40 && move_angle < 140
    MOVE_KB_WITHIN(result, move_currKnockback, move_angle, 100, 0, 100, 50, 250)
    if Equal result 1
      DynamicDiceAdd lastAttack
      Goto fastCheck
      Goto dirCheck
