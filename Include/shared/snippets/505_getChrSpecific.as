    ...
    if !(True)
      label shuffle_AI
        ...
        if Rnd < immediateTempVar
          anotherTempVar = PT_AGGRESSION * -1
          ADJUST_PERSONALITY idx_aggression anotherTempVar Rnd
          ...
          ADJUST_PERSONALITY idx_baitChance 1.5 1

          ADJUST_PERSONALITY idx_wall_chance -11 1
          ADJUST_PERSONALITY idx_wall_chance 1.5 1

          ADJUST_PERSONALITY idx_braveChance -11 1
          ADJUST_PERSONALITY idx_braveChance 1.5 1
          ...
          ADJUST_PERSONALITY idx_wall_chance 2 Rnd
          ADJUST_PERSONALITY idx_braveChance 2 Rnd
...
#snippet EVT_CHR_CHECKS
  if Equal TRAINING_MODE_OPTION -1
    ...
    if Rnd <= 0.02
      ...
      ADJUST_PERSONALITY idx_aggression 2 1
...
#snippet EVT_CHECK_DEFEND
  if currGoal < cg_edgeguard && TRAINING_MODE_OPTION <= 1 && !(Equal currGoal cg_attack_reversal) && !(CalledFrom ExecuteAttack)
    GET_CHAR_TRAIT(immediateTempVar, chr_chk_OInCombo)
    ...
    if !(Equal currGoal cg_attack_inCombo) && !(Equal currGoal cg_defend)
      ...
      if immediateTempVar <= 1 || Equal currGoal cg_bait_wait
        ...
        if !(Equal lastAttack -1)
          ...
          immediateTempVar = OEndLag * anotherTempVar * 0.5
