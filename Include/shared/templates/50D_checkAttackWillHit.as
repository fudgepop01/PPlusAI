...
// check to see if move will hit if swung now
  ...
  // tempXRange *= 0.5
  // tempYRange *= 0.5
  globTempVar = 2 - 0.3 * estFrame
  anotherTempVar = OWidth * 0.5 - globTempVar
  tempXRange += anotherTempVar
  anotherTempVar = OHurtboxSize * 0.5 - globTempVar
...
label checkCanEdgeguard
  ...
  if Equal immediateTempVar -1 || Equal anotherTempVar -1 || Equal YDistFloor -1 
    JmpNextIfLabel
  elif currGoal >= cg_edgeguard && Equal OYDistFloor -1
    IfLabel
