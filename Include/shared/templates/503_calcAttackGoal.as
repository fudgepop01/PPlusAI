...
elif Equal CurrSubaction JumpSquat || CalledFrom Shield || immediateTempVar > 5
...
elif YDistFloor > 5 && YSpeed > -0.25 && CurrAction <= hex(0x20)
elif OYDistFloor > 45
else
  aerialChecks = 0
endif
...
label check_hub
  ...
  if YDistFloor < 20 && !(Equal YDistFloor -1) 
    ...
    if !(Equal immediateTempVar 0) 
      ...
      if Equal AirGroundState 2 && XDistLE anotherTempVar
        ...
        if YDistFloor > 0
          anotherTempVar = FastFallSpeed / YDistFloor
          immediateTempVar += anotherTempVar
        endif
        ...
        anotherTempVar -= 0.1
        anotherTempVar *= 40
        anotherTempVar += calc(cs_shortHopHeight * 1.75)
  ...
  if !(Equal currGoal cg_attack_wall) && currGoal >= cg_attack
    if globTempVar > 0.16
      move_IASA *= 0.25
    else 
      globTempVar = ODamage * 0.12
      move_IASA -= globTempVar
    endif
  endif
  ...
  if move_damage <= 0
    ...
    rollWeight = immediateTempVar * move_xRange * 0.45
    move_IASA *= 0.75
