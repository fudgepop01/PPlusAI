...
if TRAINING_MODE_OPTION >= 0
  ...
  anotherTempVar = 1
  ADJUST_PERSONALITY idx_aggression immediateTempVar anotherTempVar
  ADJUST_PERSONALITY idx_bait_dashAwayChance immediateTempVar anotherTempVar
  ADJUST_PERSONALITY idx_bait_wdashAwayChance immediateTempVar anotherTempVar
  ADJUST_PERSONALITY idx_baitChance immediateTempVar anotherTempVar
  ADJUST_PERSONALITY idx_braveChance immediateTempVar anotherTempVar
  ADJUST_PERSONALITY idx_circleCampChance immediateTempVar anotherTempVar
  ...
  ADJUST_PERSONALITY idx_wall_chance immediateTempVar anotherTempVar
  // ADJUST_PERSONALITY idx_reaction_time 100 1
  
  ClearStick
  if Equal TRAINING_MODE_OPTION TRAIN_ATTACK
    ADJUST_PERSONALITY idx_aggression 100 anotherTempVar
    ADJUST_PERSONALITY idx_braveChance 100 anotherTempVar
    Seek initHitPredictValues
  elif Equal TRAINING_MODE_OPTION TRAIN_RUN
    ADJUST_PERSONALITY idx_circleCampChance 100 anotherTempVar
    ADJUST_PERSONALITY idx_baitChance 100 anotherTempVar
    Seek initHitPredictValues
...
if Equal currGoal cg_recover && Equal YDistFloor -1
  CallI RecoveryHub
elif Equal CurrAction hex(0x10)
  CallI RecoveryHub
...
anotherTempVar -= 0.25
anotherTempVar *= -3 * immediateTempVar 
...
anotherTempVar = PT_AGGRESSION * 0.675
immediateTempVar += anotherTempVar
DynamicDiceAdd dslot0 cg_bait_attack immediateTempVar
immediateTempVar = PT_BAIT_DASHAWAYCHANCE * 1.2
...
if immediateTempVar < 0.02 && CHANCE_MUL_LE PT_AGGRESSION 0.02
  ...
  if YDistFloor < 25
    ...
    immediateTempVar *= 7 * PT_WALL_CHANCE
...
elif Equal HitboxConnected 1 || Equal PrevAction hex(0x3C) || Equal immediateTempVar 1
  currGoal = cg_attack_reversal
endif
goalY = BBoundary
if Equal currGoal -1
...
if Equal currGoal cg_bait_wait && LevelValue >= LV5
  if CurrAction >= hex(0x3) && CurrAction <= hex(0xA)
    scriptVariant = sv_wavedash_neutral
    CallI Wavedash
  endif
  label waitSetup
