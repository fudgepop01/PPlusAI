...
if currGoal >= cg_circleCamp && currGoal < calc(cg_circleCamp + 1)
  ...
  ADJUST_PERSONALITY idx_circleCampChance -0.002 1
  ...
  ADJUST_PERSONALITY idx_baitChance -0.001 1
  if LevelValue <= LV3
    currGoal = cg_attack
    Return
  endif
  ...
  if immediateTempVar <= 1 && OAnimFrame > globTempVar
    ...
    globTempVar = OTopNY - TopNY
    if globTempVar > 45 || OYDistBackEdge < -20 && Equal AirGroundState 1 && CHANCE_MUL_LE PT_AGGRESSION 1.25
      currGoal = cg_attack
      Return
    endif
  ...
  Goto getDist
  GetCommitPredictChance anotherTempVar
  if immediateTempVar >= 55 || anotherTempVar < 0.12 || CHANCE_MUL_LE PT_BAITCHANCE 0.1 || anotherTempVar > 0.18
    shouldApproach = 1
  endif 
  ...
  if XDistLE anotherTempVar && Equal AirGroundState 1 && Equal currGoal cg_bait_dashdance
    ...
    elif CHANCE_MUL_LE PT_BAIT_WDASHAWAYCHANCE 0.2 && LevelValue >= LV7
  ...
  GET_CHAR_TRAIT(immediateTempVar, chr_chk_OInCombo)
  if currGoal < cg_attack_reversal
    JmpNextIfLabel
  elif Equal OIsOnStage 1 && currGoal >= cg_edgeguard
    IfLabel
  ...
  predictAverage anotherTempVar man_OXHitDist
  anotherTempVar *= 0.5
  if XDistLE anotherTempVar && Equal currGoal cg_attack_wall && CHANCE_MUL_LE PT_AGGRESSION 0.2
    currGoal = cg_attack
    lastAttack = -1
    Return
  endif
  anotherTempVar *= 2
  anotherTempVar += 20
  if XDistLE anotherTempVar && currGoal < cg_attack_reversal && !(Equal currGoal cg_attack_wall)
    GetCommitPredictChance immediateTempVar
  ...
  predictAverage immediateTempVar man_OXHitDist
  immediateTempVar += 10
  if {Equal OCurrAction hex(0x49) && currGoal < cg_edgeguard} && {XDistLE immediateTempVar && !(Equal OAirGroundState 1)}
  ...
  if currGoal >= cg_edgeguard
    ...
    if immediateTempVar > anotherTempVar
      GET_CHAR_TRAIT(immediateTempVar, chr_chk_OInCombo)
      if Equal immediateTempVar 1 || currGoal >= cg_edgeguard
...
label shuffleBaitCamp
  MOD immediateTempVar GameTimer 350
  if immediateTempVar >= 335
    ADJUST_PERSONALITY idx_circleCampChance -0.4 1
    ADJUST_PERSONALITY idx_circleCampChance -0.02 ODamage
    if Rnd < 0.25
      Call MainHub
    endif
    currGoal = cg_bait
...
label forceChangeGoal
  ...
  elif Equal currGoal cg_circleCamp && Rnd < 0.75
    XGoto CircleCampGoal
  else
    XGoto RandomizeGoal
  endif
