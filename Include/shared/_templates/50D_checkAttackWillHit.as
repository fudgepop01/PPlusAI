...
if globTempVar < 8
elif {OCurrAction >= hex(0x4E) && OCurrAction <= hex(0x52) && globTempVar < 26} || {OCurrAction >= hex(0x60) && OCurrAction <= hex(0x61) && globTempVar < 19} || {OCurrAction >= hex(0x1f) && OCurrAction <= hex(0x20) && globTempVar < 19} || {{Equal OCurrAction hex(0x1e) || Equal OCurrAction hex(0x8e) || Equal OCurrAction hex(0x90) || Equal OCurrAction hex(0x91)} && globTempVar < 15} || {Equal OCurrAction hex(0x21) && globTempVar < 29} || {{Equal OCurrSubaction hex(0xD9) || Equal OCurrSubaction hex(0x30)} && globTempVar < 33} || {{Equal OCurrSubaction hex(0xDE) || Equal OCurrSubaction hex(0xDD)} && globTempVar < 61}
  Return
endif
...
$ifSpecialAttack() 
  ...
  if {move_centerX < 0 && OPos < 0} || {move_centerX > 0 && OPos > 0}
  elif True
    move_centerX *= -1
  endif
...
label checkIfAirViable
  if Equal CurrAction hex(0xA)
    ...
    moveDir *= Direction * OPos
    $ifSpecialAttack()
      if NoJumpPrevFrame
        Button X
      endif
    elif Equal moveDir -1
      scriptVariant = sv_noNavigation
      if !(Equal CurrAction hex(0x7)) && !(Equal CurrAction hex(0x6))
        ClearStick
        Stick -1
      endif
