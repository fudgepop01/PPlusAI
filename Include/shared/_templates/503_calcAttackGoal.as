...
if {Equal currGoal cg_attack_wall && YDistFloor > 25} || {Equal currGoal cg_attack_wall && YDistFloor < 0}
  currGoal = cg_attack
elif {currGoal >= cg_attack && currGoal <= calc(cg_attack + 1)} || {Equal scriptVariant sv_campAttack || Equal currGoal cg_bait_attack || Equal currGoal cg_bait_shield}
else
  currGoal = cg_attack
endif
...
// anotherTempVar *= 0.2
if {CurrAction >= hex(0x1A) && CurrAction <= hex(0x1D)} || {Equal CurrSubaction JumpSquat || CalledFrom Shield} || immediateTempVar > 5 || {YDistFloor > 8 && XDistLE 15} || {OCurrAction >= hex(0x42) && OCurrAction <= hex(0x64) && immediateTempVar > 20} || {CurrAction >= hex(0x73) && CurrAction <= hex(0x7B)}}
...
label check_hub
  ...
  if move_damage <= 0
    ...
    immediateTempVar += 1
    immediateTempVar *= immediateTempVar
    rollWeight = immediateTempVar * move_xRange * 0.10
    move_IASA *= 0.75
      ...
      if globTempVar >= anotherTempVar
        ...
        if currGoal < cg_edgeguard
          ...
          if var11 > 0 && Equal AirGroundState 2 || YDistFloor < OFramesHitstun && YDistFloor > -1 && anotherTempVar < 35
            if {Equal Direction OPos && dirX >= 0} || {!(Equal Direction OPos) && dirX <= 0} 
              rollWeight *= 1.75
